...
export default function Features() {
  const features = [
    {
      // Icon: Petir / Speedometer (Representasi Kecepatan)
      icon: (
        <div className="w-20 h-20 rounded-full bg-blue-100 flex items-center justify-center mb-6 shadow-sm group-hover:scale-110 transition-transform duration-300">
    ...
    {
      // Icon: Otak / Chip (Representasi AI & Akurasi)
      icon: (
        <div className="w-20 h-20 rounded-full bg-violet-100 flex items-center justify-center mb-6 shadow-sm group-hover:scale-110 transition-transform duration-300">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-10 h-10 text-violet-600">
    ...
    {
      // Icon: Dokter / Medis (Representasi Support Tenaga Medis)
      icon: (
        <div className="w-20 h-20 rounded-full bg-emerald-100 flex items-center justify-center mb-6 shadow-sm group-hover:scale-110 transition-transform duration-300">
        ...
        <div className="relative bg-slate-800/30 border border-slate-700/60 rounded-2xl p-6 md:p-10 backdrop-blur-sm">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {features.map((feature, index) => (
              <div
                key={index}
                // Tambahkan class 'group' agar icon bisa animasi saat card di-hover
                className="bg-white p-8 rounded-xl shadow hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col items-center text-center min-h-[260px] group border border-slate-200"
              >
                {/* Render Icon langsung dari object */}
                {feature.icon}

                <h3 className="text-xl font-semibold text-slate-800 mb-2">{feature.title}</h3>
                <p className="text-slate-500 text-sm max-w-md leading-relaxed">{feature.desc}</p>
              </div>
            ))}
          </div>
        </div>
