      ...
      <div className="max-w-4xl mx-auto px-6 md:px-12 relative z-10">
        <div className="panel-translucent rounded-2xl p-8 flex flex-col items-center gap-8 bg-slate-800/50 backdrop-blur-sm border border-slate-700">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            >
              {isLoading ? <span className="animate-spin h-5 w-5 border-2 border-white border-t-transparent rounded-full"></span> : null}
                        ...
                        {singleResult.probabilities?.map((prob, idx) => (
                          <div key={idx} className="flex flex-col gap-1">
                            ...
                            <div className="w-full bg-slate-700 rounded-full h-2">
                              <div
                                className={`h-2 rounded-full ${prob.class_name === singleResult.predicted_class ? "bg-blue-500" : "bg-slate-500"}`}
                                style={{
          ...
          {multiFiles && multiFiles.length > 0 && (
            <div className="w-full mt-8 pt-8 border-t border-slate-700">
              <div className="flex items-center justify-between mb-3">
                <div className="text-slate-300">{multiFiles.length} file(s) ready</div>
                <div className="flex items-center gap-2">
                  {csvUrl && (
                    <a href={csvUrl} download="hasil_klasifikasi.xlsx" className="mt-4 px-4 py-2 bg-green-600 rounded-md text-white">
                      Download Excel
                    </a>
                  )}
